//Variables para indetificar la letra en el NIF
var NIF_Type = {
    'A':'Sociedad Anónima',
...
//Variable para identificar el nuero de la Provincia
var CIF_Prov = {
    '00':'No Residente',
    '01':'Álava',
    '02':'Albacete',
...
//Regla del valor introducido en el campo nif
String.prototype.isNIF=function(){
    return /^(\d{7,8})([A-HJ-NP-TV-Z])$/.test(this) && 
    ("TRWAGMYFPDXBNJZSQVHLCKE"[(RegExp.$1%23)]==RegExp.$2);
 };

 /**
 * comprovamos si el valor utilizado en el campo es un valor correcto segun la regla
 * @param {*} idInput 
 * @param {*} idForm 
 */
function CheckNifCif(idInput,idForm) {
    if(chNIF($(idInput).val())){
...
/**
 * Chequeamos que el campo de NIF/CIF/NIE es un valor valido.
 * @param {*} nif 
 */
function chNIF(input) {
    //formatamos el valor a un String controlable
    nif = input.toUpperCase().replace(/[\s\-]+/g,'');
